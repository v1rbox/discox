...
class cmd(Command):
    ...
    async def execute(self, arguments: list[str], message: discord.Message) -> None:
        channel_id = await self.db.raw_exec_select(
        ...
        if not message.reactions[0] in ["✅", "❌"]:
            for i in range(len(message.embeds[0].fields)):
                embed.add_field(
                    ...
                    value=f"{message.reactions[i].count - 1} votes out of {sum([x.count for x in message.reactions])}. ({self.calc_percent(message.reactions[i].count, sum([x.count for x in message.reactions]) - len(message.reactions))}%)",
                    inline=False,
                )
        else:
            embed.add_field(
                name="True",
                value=f"{message.reactions[0].count - 1} votes out of {sum([x.count for x in message.reactions])}. ({self.calc_percent(message.reactions[0].count, sum([x.count for x in message.reactions]) - len(message.reactions))}%)",
                inline=False,
            )
            embed.add_field(
                name="False",
                value=f"{message.reactions[1].count - 1} votes out of {sum([x.count for x in message.reactions])}. ({self.calc_percent(message.reactions[1].count, sum([x.count for x in message.reactions]) - len(message.reactions))}%)",
                inline=False,
            )
