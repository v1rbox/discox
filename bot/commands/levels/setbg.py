...
class cmd(Command):
    ...
    async def execute(self, arguments, message) -> None:
        if arguments[0] == "":
            ...
            return
        view = OptionView()
        await message.channel.send("Are you sure you want to set this background?", view=view)
        answer = await view.get_answer()
        if answer:
            cursor = await self.db.cursor()
            await cursor.execute(f"UPDATE levels SET bg = '{arguments[0]}' WHERE user_id = '{message.author.id}'")
