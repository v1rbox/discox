...
class cmd(Command):
    ...
    async def execute(self, arguments, message) -> None:
        ...
        if not len(rgb) == 3:
            raise ValueError("RGB malformed. Expecting 3 of them to be digits between 0 and 255.")
        option = Confirm()
        ...
        answer = option.value
        if answer:
            await self.db.raw_exec_commit("UPDATE levels SET font_color = ? WHERE user_id = ?", (' '.join([str(i) for i in rgb]), message.author.id))
