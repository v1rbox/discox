...
class cmd(Command):
    ...
    async def execute(self, arguments, message) -> None:
        try:
            ...
            embed = Embed(
                title="Minesweeper", description="Number of mines must be an int"
            )
        ...
        if int(arguments[0]) > 24:
            embed = Embed(
                title="Minesweeper",
                description="Number of mines must be smaller than 25",
            )
            await message.reply(embed=embed)
            return
        embed = Embed(
            title="Minesweeper",
            description=f"Dont get rekt! {arguments[0]} mines have been placed!",
        )
