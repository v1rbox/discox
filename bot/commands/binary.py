...
class cmd(Command):
    ...
    def decode_binary(self, string) -> str:
        ...
        try:
           ...
           raise RuntimeError("An error occured decoding the binary sequence - might not be a valid one.")

    async def execute(self, arguments, message) -> None:
        # Get the option - either "encode" or "decode".
        if arguments[0] not in ("encode", "decode"):
            raise KeyError(f"Invalid option: {arguments[0]} - valid options are 'encode' and 'decode'.")
