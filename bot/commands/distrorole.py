    ...
    async def execute(self, arguments, message) -> None:
        ...
        #
          ...
          for role in server_roles_names:
            if role in self.whitelist:
              leaderboard.append({"role": role, "count" : len(self.getRole(message,role).members)})
        ...
        #
          ...
          embed = Embed(title="Distro", description="**<@298308374590652416> is leet linux chad** \n\n ***Fedora > Any other distro***")
          await message.channel.send(embed=embed)
        ...
        #
          ...
          if arguments[1] not in self.whitelist or arguments[1] not in user_roles_names:
            embed = Embed(title="Distro", description=f"**`{name}` does not have that distro role, or `{arguments[1]}` is not whitelisted**\n\n*To your see current distro roles, use:* \n`v!distro roles`\n\n*To see whitelisted distro roles, use:*\n`v!distro whitelist`")
            await message.channel.send(embed=embed)
            return
          # Removes role from user
          role = self.getRole(message, arguments[1])
          await message.author.remove_roles(role)
          embed = Embed(title="Distro", description=f"**`{name}` has been removed from the `{role.name}` distro role.**")
          await message.channel.send(embed=embed)
