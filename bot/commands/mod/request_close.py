...
class cmd(Command):
    ...
    async def execute(self, arguments, message) -> None:
        ...
        if str(reaction) == '👍':
            ...
            await cursor.execute(f"UPDATE sqlite_sequence SET seq = ? WHERE name = 'request'", (res[0][1] - 1, ))
            ...
            for i in range(0, len(res)):
                if i + 1 == int(res[i][0]):
                    continue
                await cursor.execute("UPDATE request SET Number_id = ? WHERE Number_id = ?", (int(i) + 1, res[i][0]))

            await db.commit()
            embed = Embed(
                    title="The data has successfully been deleted!")
            await message.channel.send(embed=embed)

        elif str(reaction) == '👎':
            embed = Embed(
                    title="The request has successfully been ignored!")
            await message.channel.send(embed=embed)
