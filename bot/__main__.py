...
async def parse_types(
    ...
    usage_raw = re.findall(r"<.*?>|\[(.*?)(?::(\w+))?\]", usage)
    usage_parsed = []
    for i in usage_raw:
        if i[1] is None:
            usage_parsed.append([i[0], "str"])
        else:
            usage_parsed.append([i[0], i[1]])
    result = []
    for i in range(len(arguments)):
        match usage_parsed[i][1]:
            case "member":
                try:
                    user = message.guild.get_member_named(arguments[i])
                    assert user is not None
                except AssertionError:
                    try:
                        user = await message.guild.fetch_member(arguments[i])
