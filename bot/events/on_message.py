    ...
    async def execute(self, message) -> None:
        ...
        if not message.author.bot:
            if self.lastMsgAuthorId != message.author.id:
                ...
                await cursor.execute(f"SELECT exp, level FROM levels WHERE user_id = '{message.author.id}'")
                ...
                if result is None:
                    ...
                    if result[0] + 1 >= result[1] * 25 + 100:
                        ...
                        await message.channel.send(f"Congratulations {message.author.mention}, you just advanced to level {result[1] + 1}!")
