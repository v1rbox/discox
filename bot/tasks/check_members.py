...
class TaskLoop(Task):
    ...
    async def execute(self) -> None:
        ...
        # clearing someone exited this time
        registered = [int(x[0]) for x in (await self.db.raw_exec_select("SELECT user_id FROM levels"))]
        for member in [x for x in self.bot.get_all_members() if not x.bot]:
            if member.id in registered:
                registered.remove(member.id)
        
        if registered:
            await self.db.raw_exec_commit("DELETE FROM levels WHERE user_id IN ({})".format(", ".join(["?"] * len(registered))), registered)
